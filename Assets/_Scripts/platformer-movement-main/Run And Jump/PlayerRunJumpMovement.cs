...
public class PlayerRunJumpMovement : MonoBehaviour
{
	...
	[FormerlySerializedAs("Data")] public PlayerRunJumpData runJumpData;
	...
	private void Update()
	{
		...
		else if (RB.velocity.y < 0 && _moveInput.y < 0)
		{
			...
			RB.velocity = new Vector2(RB.velocity.x, Mathf.Max(RB.velocity.y, -runJumpData.maxFastFallSpeed));
		}
		else if (_isJumpCut)
		{
			//Higher gravity if jump button released
			SetGravityScale(runJumpData.gravityScale * runJumpData.jumpCutGravityMult);
			RB.velocity = new Vector2(RB.velocity.x, Mathf.Max(RB.velocity.y, -runJumpData.maxFallSpeed));
		}
		else if ((IsJumping || IsWallJumping || _isJumpFalling) && Mathf.Abs(RB.velocity.y) < runJumpData.jumpHangTimeThreshold)
		{
    ...
    private void Run(float lerpAmount)
	{
		...
		if ((IsJumping || IsWallJumping || _isJumpFalling) && Mathf.Abs(RB.velocity.y) < runJumpData.jumpHangTimeThreshold)
		{
		...
		if(runJumpData.doConserveMomentum && Mathf.Abs(RB.velocity.x) > Mathf.Abs(targetSpeed) && Mathf.Sign(RB.velocity.x) == Mathf.Sign(targetSpeed) && Mathf.Abs(targetSpeed) > 0.01f && LastOnGroundTime < 0)
		{
	...
	private void Slide()
	{
		...
		float speedDif = runJumpData.slideSpeed - RB.velocity.y;	
